...
ggplot(dateDF, aes(x=dates, y = points, color = periods, group = grp))+geom_line(alpha = 0.8, size = 1)+theme_tufte()+ theme(axis.title.y=element_blank(), axis.text.y=element_blank(),
        axis.ticks.y=element_blank(),axis.title.x=element_blank(), plot.caption = element_text(hjust = 0.5,  face= "italic"))+
#  scale_x_date(date_labels = "%b %d",date_breaks = "2 weeks", limits = c(init-10,end+10))+
  scale_x_date(date_labels = "%b %d", breaks = c(init, as.Date(startDate), as.Date(interventionDate), as.Date(observationDate), end), limits = c(init-10,end+10))+
